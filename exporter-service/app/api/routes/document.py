...
async def export_document_excel(db:MongoDB, background_tasks: BackgroundTasks,user:CurrentUser,document_id: str = Query(..., description="The ID of the document to download"))-> FileResponse:
  """
  ...
  return FileResponse("output.xlsx", filename=document_xlsx_file_path, media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

@router.get("/{workflow_id}")
async def list_documents(db:MongoDB,user:CurrentUser,workflow_id: int = Query(..., description="The ID of the workflow for which to list documents")):
  """

      ...
      <div className="border-2 rounded-lg p-5 flex flex-col justify-between min-h-[600px]">
        <div>
          ...
          {files.map((file, index) => (
            <ProcessedFileCard key={index} fileId={file.id} workflowId={file.workflow_id} dox_id={file.dox_id} index={index} name={file.name} date={file.uploaded_at} processed_status={file.process_status} onDownload={handleDownloadFiles} />
          ))}
        </div>
      </div>
    </div>
